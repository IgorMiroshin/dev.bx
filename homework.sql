...
(
    ...
    LANG_ID int,
    PRIMARY KEY (ID),
    FOREIGN KEY FK_MA_LANG (LANG_ID)
        REFERENCES lang (ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT
);

INSERT INTO movie_title (NAME)
SELECT TITLE
FROM movie;
...
ALTER TABLE movie
    ADD FOREIGN KEY FK_MA_MOVIE_TITLE (movie_title_id)
        REFERENCES movie_title (ID)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT;

UPDATE movie JOIN movie_title ON movie.TITLE = movie_title.NAME
SET movie.movie_title_id = movie_title.ID
WHERE movie.movie_title_id is NULL;
